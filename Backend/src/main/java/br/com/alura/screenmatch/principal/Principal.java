...
public class Principal {
    ...
    private void buscarEpisodioPorSerie() {
        ...
        var serieEncontrada = serieOpt.get();
        List<DadosTemporada> temporadas = new ArrayList<>();

        int totalTemporadas = serieEncontrada.getTotalTemporadas() != null
                ? serieEncontrada.getTotalTemporadas()
                : 0;

        // Busca dados das temporadas via API
        ...
        // Pega todos os episódios já cadastrados dessa série
        List<Episodio> episodiosExistentes = episodioRepository.findBySerie_Id(serieEncontrada.getId());

        // Cria Set para verificar duplicados rapidamente (temporada + numero)
        Set<String> episodiosJaCadastrados = episodiosExistentes.stream()
                .map(ep -> ep.getTemporada() + "-" + ep.getNumeroEpisodio())
                .collect(Collectors.toSet());

        // Mapear episódios novos a partir dos dados da API
        List<Episodio> episodiosNovos = temporadas.stream()
                .flatMap(temporada -> temporada.episodios().stream()
                        .map(epDados -> {
                            Episodio ep = new Episodio();

                            ep.setSerie(serieEncontrada);
                            ep.setTemporada(temporada.numero());
                            ep.setNumeroEpisodio(ep.getNumeroEpisodio());
                            ep.setTitulo(ep.getTitulo());
                            ep.setAvaliacao(ep.getAvaliacao());
                            ep.setDataLancamento(ep.getDataLancamento()); // ajuste conforme sua classe Episodio e DadosEpisodio

                            return ep;
                        })
                )
                // Filtra só episódios que não existem no banco
                .filter(ep -> !episodiosJaCadastrados.contains(ep.getTemporada() + "-" + ep.getNumeroEpisodio()))
                .collect(Collectors.toList());

        // Salva só os episódios novos
        episodioRepository.saveAll(episodiosNovos);

        System.out.println(episodiosNovos.size() + " episódios novos adicionados!");
    }
